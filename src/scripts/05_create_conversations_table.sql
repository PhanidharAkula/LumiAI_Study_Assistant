-- Conversations: stored Q&A history with the AI assistant.
-- Idempotent: safe to re-run. (Note: this script no longer begins with
-- DROP TABLE — re-running it must never destroy existing conversation history.)
...
-- Create policies (drop-then-create so re-running is safe)
