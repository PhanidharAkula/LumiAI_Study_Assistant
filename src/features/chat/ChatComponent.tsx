...
// Tailwind class groups — 1:1 port of ChatComponent.css (the round/pill header
// buttons and the per-history-item action buttons share a base each).
const HEADER_BTN_BASE =
  "h-[45px] w-[45px] rounded-full flex justify-center items-center shadow-[0px_2px_0_#000] border-[1.5px] border-solid border-ink cursor-pointer [transition:transform_0.2s_ease] p-0 hover:[transform:translateY(-4px)] [&_svg]:w-5 [&_svg]:h-5";
const HISTORY_ACTION_BASE =
  "w-[30px] h-[30px] rounded-full border border-solid border-ink shadow-[0px_1px_0_#000] flex items-center justify-center cursor-pointer [transition:all_0.2s_ease] hover:[transform:translateY(-3px)]";
...
const ChatComponent = ({
  ...
  const [, setClassData] = useState<any>(null);
  ...
  const fetchChatHistory = async () => {
    ...
    try {
      ...
      const processedHistory: ChatHistoryItem[] = (data as any[]).map((conv) => {
    ...
    try {
      ...
      if (data) {
        ...
        for (const conv of sessionConversations as any[]) {
          ...
          let storedMessages: StoredMessageMeta[] = [];
          try {
      ...
      if (user) {
        try {
          ...
          if (currentConversationId) {
            ...
            const messagesMetadata: StoredMessageMeta[] = [];
  ...
  if (initialLoading) {
    return (
      <motion.div
        className="fixed inset-0 bg-cream z-[1000] flex flex-col overflow-hidden pt-[30px] max-md:p-2.5"
  ...
  return (
    ...
    >
      <motion.div
        className="p-[30px] flex items-center gap-[15px] bg-transparent fixed top-0 left-0 z-[110] w-full max-[1024px]:p-[18px] max-[1024px]:bg-cream max-md:right-0 max-md:p-[15px] max-md:z-[130]"
      ...
      >
        <div className="flex gap-[15px]">
          <div className="relative">
            <button
              className={`history-button ${HEADER_BTN_BASE} ${
                showHistory ? "bg-sage text-[#8B5CF6]" : "bg-white text-ink"
              }`}
            ...
            <AnimatePresence>
              {showHistory && (
                <motion.div
                  className="absolute top-[55px] left-0 w-[300px] max-h-[70dvh] bg-white rounded-xl z-[100] flex flex-col overflow-hidden max-md:w-[280px] max-md:right-[-20px] max-[480px]:right-[-100px]"
                ...
                >
                  <div className="py-[15px] px-5">
                    <h3 className="m-0 text-[x-large] font-semibold">
                      Chat History
                    </h3>
                  </div>
                  <div className="overflow-y-auto py-2 px-[15px] max-h-[calc(70dvh-55px)] [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                    {historyLoading ? (
                      <div className="p-5 flex flex-col items-center justify-center text-muted">
                        <div className="spinner"></div>
                        <p>Loading history...</p>
                      </div>
                    ) : chatHistory.length === 0 ? (
                      <div className="p-5 flex flex-col items-center justify-center text-muted">
                        <div className="w-[50px] h-[50px] rounded-full bg-sage flex items-center justify-center mb-2.5 border-[1.5px] border-solid border-ink">
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="24"
                            height="24"
                        ...
                        <p className="m-0 text-[14px] text-center">
                      ...
                      chatHistory.map((conv) => (
                        <div
                          key={conv.id}
                          className="group rounded-[10px] bg-black/10 mb-2.5 p-2.5 cursor-pointer [transition:all_0.2s_ease] relative hover:[transform:translateY(-3px)] hover:scale-[1.01]"
                        >
                          {editingTitle === conv.id ? (
                            <div className="py-1.5 px-0">
                              <input
                                ...
                                className="w-full py-1.5 px-2.5 rounded-md border-[1.5px] border-solid border-ink text-[15px] font-medium bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)] focus:outline-none focus:border-[#8B5CF6] focus:shadow-[0_0_0_2px_rgba(139,92,246,0.2)]"
                              />
                            </div>
                          ) : (
                            <>
                              ...
                              >
                                <p className="mt-0 mx-0 mb-0.5 text-[14px] font-semibold text-ink">
                                  {conv.title || "New Conversation"}
                                </p>
                                <p className="mt-0 mx-0 mb-1.5 text-[13px] leading-[1.3] text-muted whitespace-nowrap overflow-hidden text-ellipsis">
                                  {conv.displayQuestion}
                                </p>
                              </div>
                              <div className="absolute bottom-3 right-3 flex gap-2 opacity-0 [transition:opacity_0.2s_ease] group-hover:opacity-100">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleTitleEdit(conv);
                                  }}
                                  className={`${HISTORY_ACTION_BASE} bg-sage`}
                                  title="Edit title"
                                >
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                ...
                                <button
                                  ...
                                  className={`${HISTORY_ACTION_BASE} bg-[#EF4444] [&_svg]:stroke-white`}
                              ...
                              <div className="flex justify-between items-center text-[11px] text-muted">
                                ...
                                {conv.classes && (
                                  <span className="bg-sage py-0.5 px-2 rounded-[10px] text-[11px]">
                                    {conv.classes.name}
                                  </span>
                                )}
                              </div>
          ...
          {messages.length > 0 && (
            <button
              className={`${HEADER_BTN_BASE} bg-white text-ink`}
      ...
      <div className="flex flex-1 relative overflow-hidden">
        ...
        >
          {documents.length > 0 && (
            <motion.div
              className="py-[15px] px-5 border-0 border-b-[1.5px] border-solid border-ink"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1, duration: 0.3 }}
            >
              <div className="flex justify-between items-center mb-2.5">
                <p>Select documents for context</p>
          ...
          <motion.div
            className="w-[55%] flex-1 overflow-y-auto flex flex-col scroll-smooth [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden p-5 pb-2.5 max-[1024px]:w-full max-[1024px]:p-3.5 max-md:w-full max-md:pt-2.5 max-md:px-3 max-md:pb-20 max-md:mt-16"
          ...
          >
            {messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center text-muted p-10">
                <div className="w-20 h-20 flex items-center justify-center rounded-full bg-sage border-[1.5px] border-solid border-ink shadow-[0px_2px_0_#000] mb-5">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="40"
                    height="40"
                ...
                <h3 className="my-2.5 mx-0 text-[24px] font-semibold text-ink">
          ...
          <div className="w-[55%] bg-cream max-[1024px]:w-full max-md:w-full max-md:fixed max-md:left-0 max-md:right-0 max-md:bottom-0 max-md:z-[120] max-md:py-2 max-md:px-3">
            <AnimatePresence>
